...
class AffranchigoLibCase:
    ...
    def select_time_in_selectors(self):
        select_time_selectors = [
            "#\\[g0_p265\\|0_r2055\\[0\\]\\] input-component select",
            "#g0_p265\\|0_r2055\\[0\\] input-component select"
        ]
        try:
    ...
    def submit_liberte(self):
        """Soumet le formulaire de liberté."""
        try:
            logger.debug("Attente de la visibilité du bouton de soumission.")
            ...
            submit_button.click()
            logger.debug("Bouton de soumission cliqué.")
            ...
            try:
                ...
                logger.info("Retour à l'URL de départ réussi.")
            except Exception as e:
                logger.critical(f"Erreur lors de la navigation vers l'URL de départ : {e}")
                self.driver.save_screenshot("debug_screenshot_erreur_retour.png")
        except TimeoutException:
            logger.error("Le bouton de soumission n'a pas été trouvé dans les temps")
            self.driver.save_screenshot("debug_screenshot_erreur_clic.png")
        except Exception as e:
            logger.critical(f"Erreur lors de la soumission du formulaire : {e}")
            self.driver.save_screenshot("debug_screenshot_erreur_soumission_lib.png")
    ...
    def handle_case_lib(self, numero_contrat, dictionnaire):
        ...
        logger.debug(f"Début de traitement du cas Liberté pour le contrat {numero_contrat}")
